...
import { sanitizeLogText } from './structured-log'
...
export class DefaultExecutionLogger implements ExecutionLogger {
    ...
    private log(level: LogLevel, phase: LogPhase, message: string, data?: Record<string, unknown>): void {
        ...
        if (this.verbose || level === 'error') {
            ...
            if (safeData && Object.keys(safeData).length > 0) {
                ...
                logFn('  └─', JSON.stringify(safeData, null, 2).replace(/\n/g, '\n     '))
            } else {
                logFn(formattedMessage)
            }
        }
    ...
    private sanitizeLogData(data: Record<string, unknown>): Record<string, unknown> {
        ...
        const sanitizeValue = (key: string, value: unknown): unknown => {
            ...
            if (normalizedKey === 'messages' && Array.isArray(value)) {
                return value.map(message => {
                    if (typeof message !== 'object' || message === null) return { type: typeof message }
            ...
            if (sensitiveBodyKeys.has(normalizedKey) || normalizedKey === 'contentPreview'.toLowerCase()) {
            ...
            if (normalizedKey === 'error' || normalizedKey === 'errormessage') {
                return typeof value === 'string' ? sanitizeLogText(value, [process.env.ARK_API_KEY ?? '']) : { type: typeof value }
            }

            if (Array.isArray(value)) {
                return value.map(item =>
                    typeof item === 'object' && item !== null ? this.sanitizeLogData(item as Record<string, unknown>) : item
                )
            }

            if (typeof value === 'object' && value !== null) {
                return this.sanitizeLogData(value as Record<string, unknown>)
            }

            return value
        }

        return Object.fromEntries(Object.entries(data).map(([key, value]) => [key, sanitizeValue(key, value)]))
    }
